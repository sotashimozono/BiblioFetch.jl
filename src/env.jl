...
function load_config(; path::AbstractString = get(ENV, "BIBLIOFETCH_CONFIG", DEFAULT_CONFIG_PATH))
...
"""
function probe_proxy(proxy::Union{AbstractString,Nothing};
                     url::AbstractString = "https://api.crossref.org/works?rows=0",
                     timeout::Real = 5)
    try
        kw = (; connect_timeout = timeout, readtimeout = timeout, retry = false, status_exception = false)
        resp = proxy === nothing ?
            HTTP.get(url; kw...) :
...
function detect_environment(; probe::Bool = true)
    ...
    if profile_px === "" ; profile_px = nothing ; end
    ...
    store_root = expanduser(string(get(profile, "store_root",
                       get(defaults, "store_root", DEFAULT_STORE_ROOT))))
    email = let e = get(profile, "email", get(defaults, "email", nothing))
        e === nothing || e == "" ? nothing : string(e)
    end

    return Runtime(host, profile_name, proxy, source, reachable,
                   store_root, email, mode, cfg_path)
end
...
function Base.show(io::IO, ::MIME"text/plain", rt::Runtime)
    ...
    println(io, "  config          : ", rt.config_path === nothing ? "(none)" : rt.config_path)
    println(io, "  mode            : ", rt.mode)
    println(io, "  proxy           : ", rt.proxy === nothing ? "(none)" : rt.proxy,
                "  [", rt.proxy_source, "]")
    println(io, "  reachable ($(route)): ", rt.reachable)
    println(io, "  store root      : ", rt.store_root)
      print(io, "  email           : ", rt.email === nothing ? "(unset — Unpaywall will be skipped)" : rt.email)
end
